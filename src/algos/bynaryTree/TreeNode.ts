...
class TreeNode {
	...
	calculateLeftGeneretionForRightChild(
		current: TreeNode | null = this,
		count: number = 0
	): number {
		if (!current) return count;
		const leftChild = current.getLeft ? current.getLeft() : null;
		if (!leftChild) return count;
		const leftGenerations = this.calculateLeftGeneretionForRightChild(
			leftChild,
			count + 1
		);
		const rightGenerations =
			this.calculateRightGeneretionForLeftChild(leftChild);

		return Math.max(leftGenerations, count + rightGenerations + 1);
	}

	calculateRightGeneretionForLeftChild(
		current: TreeNode | null = this,
		count: number = 0
	): number {
		if (!current) return count;
		...
		const rightGenerations = this.calculateRightGeneretionForLeftChild(
			rightChild,
			count + 1
		);
		const leftGenerations =
			this.calculateLeftGeneretionForRightChild(rightChild);
	...
	getPath() {
		...
		while (current) {
			stack.push(current.getId(), current.getValue());
